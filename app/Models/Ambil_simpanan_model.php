...
class Ambil_simpanan_model extends Model
{
    ...
    public function getData(string|null $keyword = null) : object
    {
        ...
        $this
            ->groupStart()
            ->like($this->table . '.id_ambilsimpanan', $keyword)
    ...
    public function export()
    {
        ...
        ->setTitle('Ambil_simpanan Data')
        ->setSubject('Ambil_simpanan Data')
        ->setDescription('Ambil_simpanan Data '.date("Y"))
        ->setKeywords('Ambil_simpanan')
        ->setCategory('Ambil_simpanan');

        // subtitle on row 3
        $spreadsheet->setActiveSheetIndex(0)->setCellValue('A3', 'Ambil_simpanan Data '.date("Y"));
        ...
        ->setCellValue('B'.$startRowHeader, 'Id_ambilsimpanan')
        ->setCellValue('C'.$startRowHeader, 'Id_nasabah')
        ->setCellValue('D'.$startRowHeader, 'Saldo')
        ->setCellValue('E'.$startRowHeader, 'Tanggal')
        ->setCellValue('F'.$startRowHeader, 'Nominal')
        ;
        ...
        $spreadsheet->setActiveSheetIndex(0)->setCellValue('A1', "Ambil_simpanan Data");
        $spreadsheet->getActiveSheet()->getStyle('A1:'.$highestColumn.'1')->applyFromArray($this->headerStyle);

        $startRowBody = $startRowHeader+1;
        $index = 0;
        $data_ambil_simpanan = $this->orderBy($this->columnIndex, $this->order)->findAll();
        foreach ($data_ambil_simpanan as $key => $ambil_simpanan) {
            $spreadsheet->setActiveSheetIndex(0)->setCellValue('A'.$startRowBody, ++$index);
            if(isset($ambil_simpanan->id_ambilsimpanan))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('B'.$startRowBody, $ambil_simpanan->id_ambilsimpanan);
            if(isset($ambil_simpanan->id_nasabah))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('C'.$startRowBody, $ambil_simpanan->id_nasabah);
            if(isset($ambil_simpanan->saldo))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('D'.$startRowBody, $ambil_simpanan->saldo);
            if(isset($ambil_simpanan->tanggal))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('E'.$startRowBody, $ambil_simpanan->tanggal);
            if(isset($ambil_simpanan->nominal))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('F'.$startRowBody, $ambil_simpanan->nominal);
            $startRowBody++;
        };

        $spreadsheet->getActiveSheet()->setTitle('Ambil_simpanan Data '.date('Y'));
        $spreadsheet->setActiveSheetIndex(0);
        ...
        for ($row = 1; $row <= $highestRow; $row++) {                  //  Read a row of data into an array                 
            ...
            if ($startImport == TRUE) {
                ...
                if (isset($cellData[0][$startColumnIndex + 0])){
                    $insertData["id_ambilsimpanan"] = $cellData[0][$startColumnIndex + 0];
                    $doInsert = TRUE;
                }else {
                    $doInsert = FALSE;
                }
                ...
                if (isset($cellData[0][$startColumnIndex + 3])){
                    $insertData["tanggal"] = $cellData[0][$startColumnIndex + 3];
                    $doInsert = TRUE;
                }else {
                    $doInsert = FALSE;
                }
                if (isset($cellData[0][$startColumnIndex + 4])){
                    $insertData["nominal"] = $cellData[0][$startColumnIndex + 4];
                    $doInsert = TRUE;
                }else {
                    $doInsert = FALSE;
                }
                ...
                if (
                    ...
                    strtolower($cellData[0][0]) == "id_ambilsimpanan" &&
                ...
                (
                    ...
                    strtolower($cellData[0][1]) == "id_ambilsimpanan" &&
